...
from skimage.filters import threshold_yen as threshold	#or otsu
...
def process_rough(image, index):
	sigma = 1
	img_smooth = ndi.gaussian_filter(image, sigma)

	thresh = threshold(img_smooth)

	signal_raw = img_smooth > thresh

	signal = ndi.binary_fill_holes(signal_raw)

	cell_labels,n_cells = ndi.label(signal)

	del signal_raw, thresh

	return img_smooth, signal, cell_labels, n_cells, index


def process_fine(image, index):
	sigma = 1
	img_smooth = ndi.gaussian_filter(image, sigma)

	thresh = img_smooth.max()
	min_thresh = threshold(img_smooth)
	...
	cell_labels,n_cells = ndi.label(signal)
	#plt.figure()
	#plt.imshow(img_smooth, interpolation="none", cmap="gray", vmin=thresh, vmax=4900)
	#plt.show()

	while thresh > min_thresh:
		thresh -= 100
		thresh = min_thresh if thresh < min_thresh else thresh

		signal_raw = img_smooth > thresh
		signal = ndi.binary_fill_holes(signal_raw)
		cell_labels, old_labels = ndi.label(signal)[0], cell_labels

		cell_mask = cell_labels > 0
		watershed_labels = watershed(img_smooth, old_labels, mask=cell_mask, watershed_line=True)

		n = 1		# number of added labels to watershed
		for label in np.unique(cell_labels)[1:]:
			cell_mask = cell_labels == label
			cell_row, cell_col = ndi.center_of_mass(cell_labels, labels=cell_labels, index=label)
			if watershed_labels[int(cell_row)][int(cell_col)] == 0:
				watershed_labels[cell_mask] = watershed_labels.max() + n
				n += 1
			# check position already in watershed
			# if not set to wathershed_lables.max() + n  # n keeps increasing with each added label
		cell_labels = watershed_labels

			

	n_cells = cell_labels.max()
	#q.put([img_smooth, signal, cell_labels, n_cells, index])
	del thresh, min_thresh, signal_raw, old_labels, cell_mask, watershed_labels, cell_row, cell_col
	return img_smooth, signal, cell_labels, n_cells, index

def bg_detection(images, fine=False):
	"""
	...
	if fine:
		processes = [ pool.apply_async(process_fine, args=(img, i), callback=callback) for i, img in enumerate(images) ]
	else:
		processes = [ pool.apply_async(process_rough, args=(img, i), callback=callback) for i, img in enumerate(images) ]
